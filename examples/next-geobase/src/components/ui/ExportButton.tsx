...
export const ExportButton: React.FC<ExportButtonProps> = ({
  ...
  const [downloadingType, setDownloadingType] = useState<'geojson' | 'png' | 'embeddings' | null>(null);
  ...
  const handleDownloadGeoJSON = async () => {
    ...
    setDownloadingType('geojson');
    try {
      const filename = ExportUtils.generateFilename(task, provider);
      await ExportUtils.downloadGeoJSON(detections, filename);
    } finally {
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      await ExportUtils.downloadPNG(geoRawImage, filename);
      setShowDropdown(false);
    } catch (error) {
      ...
      <button
        ...
        className={`
          ...
          ${hasExportableData && !disabled
            ? 'bg-teal-600 hover:bg-teal-700 text-white border-teal-500 shadow-md hover:shadow-lg cursor-pointer'
          ...
          <div className="p-1.5">
            {hasDetections && (
              <>
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-1.5 text-left hover:bg-teal-50 rounded-md transition-colors duration-200 text-gray-700 text-sm cursor-pointer"
                >
                  {downloadingType === 'geojson' ? (
                    <svg className="w-3.5 h-3.5 text-teal-600 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <div className="font-medium text-sm">
                      {downloadingType === 'geojson' ? 'Downloading...' : 'Download GeoJSON'}
                    </div>
            ...
            {hasGeoImage && (
              <button
                ...
                disabled={downloadingType !== null}
                className="w-full flex items-center gap-2 px-2 py-1.5 text-left hover:bg-emerald-50 rounded-md transition-colors duration-200 text-gray-700 text-sm cursor-pointer"
              >
                {downloadingType === 'png' ? (
                  <svg className="w-3.5 h-3.5 text-teal-600 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                ...
                <div>
                  <div className="font-medium text-sm">
                    {downloadingType === 'png' ? 'Downloading...' : 'Reference Image'}
                  </div>
            ...
            {hasEmbeddings && (
              <button
                ...
                disabled={downloadingType !== null}
                className="w-full flex items-center gap-2 px-2 py-1.5 text-left hover:bg-purple-50 rounded-md transition-colors duration-200 text-gray-700 text-sm cursor-pointer"
              >
                {downloadingType === 'embeddings' ? (
                  <svg className="w-3.5 h-3.5 text-purple-600 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                ) : (
                ...
                <div>
                  <div className="font-medium text-sm">
                    {downloadingType === 'embeddings' ? 'Exporting...' : 'Export Embeddings'}
                  </div>
