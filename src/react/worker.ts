  ...
  try {
    switch (type) {
      case "init": {
        ...
        const { task, provider, modelId, chain_config, ...config } =
          payload as InitPayload;
        ...
        if (chain_config) {
          ...
          modelInstance = await geoai.pipeline([{ task: task! }], {
            provider,
            ...config,
          } as ProviderParams);
        }
      ...
      case "inference": {
        ...
        const inferenceArgBuilders: Record<
          string,
          (payload: InferencePayload) => any
        > = {
        ...
        const argBuilder =
          task && inferenceArgBuilders[task]
            ? inferenceArgBuilders[task]
            : defaultBuilder;
