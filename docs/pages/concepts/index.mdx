...
> Understanding the fundamental building blocks of `@geobase/geoai.js`

## Overview

`@geobase/geoai.js` extends the Hugging Face Transformers.js library to provide geospatial AI capabilities. Here are the core concepts that make this possible:

## 1. [Architecture](./concepts/model-pipeline)

The overall architecture of `@geobase/geoai.js` provides a modular framework for geospatial AI. The GeoAI Pipeline manages model execution, the Model Registry handles task configurations, and the Base Model provides the foundation for all AI models. This architecture enables extensible provider systems and supports both Transformers.js and ONNX models.
...
Choose a concept to dive deeper into the technical details, or explore the [quickstart guide](../index) to see these concepts in action. 